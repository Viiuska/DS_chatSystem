...
class ChatWindow():
    ...
    def __init__(self):
        ...
        self._master.title("Multi-user Chatroom")
        self._master['background'] = '#BEDBE6'

        self._entry_frame = tkinter.Frame(self._master)
        self._entry_frame.grid(
            row=0, column=0, padx=5, pady=5, sticky=tkinter.W + tkinter.E, columnspan=6)
        self._entry_frame.columnconfigure(0, weight=1)
        self._entry_frame['background'] = '#BEDBE6'

        self._ip_label = tkinter.Label(
            master=self._entry_frame, text="IP:", bg='#BEDBE6')
        self._ip_label.grid(row=0, column=0, padx=5, pady=5, sticky=tkinter.W)

        self._ip_var = tkinter.StringVar()
        self._e1 = tkinter.Entry(
            master=self._entry_frame, textvariable=self._ip_var)
        self._e1.grid(row=0, column=1, padx=5, pady=5, sticky=tkinter.W)

        self._ip_label = tkinter.Label(
            master=self._entry_frame, text="Port:", bg='#BEDBE6')
        self._ip_label.grid(row=0, column=2, padx=5, pady=5)

        self._port_var = tkinter.StringVar()
        self._e2 = tkinter.Entry(
            master=self._entry_frame, textvariable=self._port_var, width=15)
        self._e2.grid(row=0, column=3, padx=6, pady=5)

        self._connect_button = tkinter.Button(
            master=self._entry_frame, command=self.connect, text="Connect")
        self._connect_button.grid(
            row=0, column=4, padx=5, pady=5, sticky=tkinter.W)

        self._button_frame = tkinter.Frame(self._master)
        self._button_frame.grid(
            row=1, column=0, padx=5, pady=5, sticky=tkinter.W + tkinter.E, columnspan=6)
        self._button_frame.columnconfigure(0, weight=1)
        self._button_frame['background'] = '#BEDBE6'

        self._room1_button = tkinter.Button(
            master=self._button_frame, command=self.move_room1, text="Room 1")
        self._room1_button.grid(row=0, column=1, padx=5, pady=5)

        self._room2_button = tkinter.Button(
            master=self._button_frame, command=self.move_room2, text="Room 2")
        self._room2_button.grid(row=0, column=2, padx=5, pady=5)

        self._room3_button = tkinter.Button(
            master=self._button_frame, command=self.move_room3, text="Room 3")
        self._room3_button.grid(row=0, column=3, padx=5, pady=5)

        self._room4_button = tkinter.Button(
            master=self._button_frame, command=self.move_room4, text="Private Room")
        self._room4_button.grid(row=0, column=4, padx=5, pady=5)

        self._name_var = tkinter.StringVar()
        self._e3 = tkinter.Entry(
            master=self._button_frame, textvariable=self._name_var, width=15)
        ...
        self._confirm_button = tkinter.Button(
            master=self._button_frame, command=self.change_name, text="Nickname")
        self._confirm_button.grid(row=0, column=6, padx=5, pady=5)

        chat_frame = tkinter.Frame(self._master)
        chat_frame.grid(row=2, column=0, pady=5,
                        sticky=tkinter.W + tkinter.E, columnspan=6)
        chat_frame.columnconfigure(0, weight=1)
        chat_frame['background'] = '#BEDBE6'
        ...
        self._send_frame.grid(row=3, column=0, padx=5, pady=5,
                              sticky=tkinter.W + tkinter.E, columnspan=6)
        self._send_frame.columnconfigure(0, weight=1)
        self._send_frame['background'] = '#BEDBE6'

        self._send_var = tkinter.StringVar()
        self._e4 = tkinter.Entry(
            master=self._send_frame, textvariable=self._send_var, width=55)
        self._e4.grid(row=0, column=1, padx=5, pady=5, sticky=tkinter.W)
        self._e4.bind('<Return>', lambda x: self._message())

        self._send_button = tkinter.Button(
            master=self._send_frame, command=self._message, text="Send")
        self._send_button.grid(row=0, column=6, padx=5,
                               pady=5, sticky=tkinter.W)

        self._error_frame = tkinter.Frame(
            self._master, relief=tkinter.RIDGE, borderwidth=8)
        self._error_frame.grid(
            row=5, column=0, sticky=tkinter.W + tkinter.E, columnspan=6)
        self._error_frame.columnconfigure(0, weight=1)

        self._info_label = tkinter.Label(
            master=self._error_frame, text="No errors")
        self._info_label.grid(row=0, column=0, padx=5,
                              pady=5, sticky=tkinter.W)

        # Closing the server socket GUI
        def close():
            try:
    ...
    # Update client's present in the GUI
    ...
    def move_room4(self):
        ...
        if len(passwordlist) == 0:
            word = simpledialog.askstring(
                "Login", "Creating a new room. Enter password")
            print(word)
            ...
            if word == passwordlist[0]:
                self._listbox.delete(0, tkinter.END)
                self.activate_others(4)
                self._room4_button.config(state=tkinter.DISABLED)
                self._send_packet("room;4")
            else:
                self._info_label.config(text="Wrong password")
        ...
        if self._connect_button['text'] == "Connect":
            ...
            try:
                ...
                listeningThread = threading.Thread(
                    target=self.check_server_response)
